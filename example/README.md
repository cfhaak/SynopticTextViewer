...
## How this example satisfies the viewer requirements

The main README describes four minimal inputs the viewer needs. In this
example project, they are realised as follows:

1. **Mount container**  
   The synoptic viewer is mounted into the `div` with ID
   `synTexView-witness-container` in `html/column_viewer.html`. Its size and
   scroll behaviour are controlled by `html/css/synopticTextViewer.css`.

2. **Per‑witness HTML snippets**  
   All witness snippets live in `html/witness_snippets/` (generated by
   `pyscripts/make_snippets.py` and `xslt/generate_snippets.xsl`). Each snippet
   consists of a heading and a sequence of line elements with stable
   identifiers (`id`/`data-n`) and the CSS classes configured in
   `html/js/synopticTextViewer/column_viewer_config.js`.

3. **JSON manifest**  
   The file `html/witness_snippets/snippet_paths.json` lists all snippets and
   their metadata (filepath, title, sorting key). The viewer reads this file at
   runtime to populate dropdowns and determine which snippets to load.

4. **Configuration module**  
   The file `html/js/synopticTextViewer/column_viewer_config.js` is a
   project-local copy of the configuration class. It maps container IDs and CSS
   classes, sets defaults such as the number of columns, and provides the
   button labels and ARIA strings used by the viewer. For a new project you
   normally adapt only this file (plus the snippet/XSLT pipeline) rather than
   editing the core viewer logic.
